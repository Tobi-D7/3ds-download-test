...
namespace d7gfx {
    ...
    public:
         ...
         * @note This should only be called when calling d7gfx::Thread::initialize() before calling d7gfx::Thread::start()
         */
        Thread();
         ...
         * @note t_function needs to be of type `void` and take one (and only one) parameter of type d7gfx::Parameter
         * @warning If the thread priority is lower than the priority of the calling thread, the thread will never get executed. Use d7gfx::Thread::getCurrentPriority() to get the priority of the current thread
         */
        Thread(std::function<void(d7gfx::Parameter)> t_function, d7gfx::Parameter t_parameter = nullptr, bool t_autostart = false, bool t_detached = false, unsigned long long int t_stackSize = 4 * 1024);
        ...
        */
         ...
         * @warning If the thread priority is lower than the priority of the calling thread, the thread will never get executed. Use d7gfx::Thread::getCurrentPriority() to get the priority of the current thread
         */
        void initialize(std::function<void(d7gfx::Parameter)> t_function, d7gfx::Parameter t_parameter = nullptr, bool t_autostart = false, bool t_detached = false, unsigned long long int t_stackSize = 4 * 1024);
    ...
    private:
        struct ThreadData {
            d7gfx::Parameter m_parameter;
            std::function<void(d7gfx::Parameter)> m_function;
        ...
        bool m_started;
        std::atomic<bool> m_running;
        d7gfx::Thread::ThreadData m_data;
