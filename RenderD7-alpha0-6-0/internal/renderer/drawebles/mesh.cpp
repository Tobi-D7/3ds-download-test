...
namespace d7gfx {
    ...
    void Mesh::addPolygon(d7gfx::Mesh::Polygon t_polygon) {
        d7gfx::Mesh::Polygon::Vertex vertex0 = t_polygon.getVertex(0),
    ...
    void Mesh::setPosition(d7gfx::Vector3f t_position) {
        m_posX = t_position.x;
        m_posY = t_position.y;
        m_posZ = t_position.z;
    }
    ...
    void Mesh::setMaterial(d7gfx::Material& t_material) {
        m_material = t_material;
    }

    d7gfx::Material& Mesh::getMaterial() {
        return m_material;
    }
    ...
    void Mesh::draw(d7gfx::RenderContext t_context) {
        if (t_context.getMode() == d7gfx::RenderContext::Mode::Spatial) {
            ...
            BufInfo_Add(bufInfo, m_vbo, sizeof(d7gfx::Mesh::Polygon::Vertex), 3, 0x210);

            // update the uniforms
            C3D_FVUnifMtx4x4(GPU_VERTEX_SHADER, t_context.getModelUniform(),  &t_context.getModelMatrix());

            // draw the VBO
            C3D_DrawArrays(GPU_TRIANGLE_STRIP, 0, m_vertices.size());
        }
    }
    ...
    void Mesh::updateVBO() {
        linearFree(m_vbo);
        m_vbo = static_cast<d7gfx::Mesh::Polygon::Vertex*>(linearAlloc(m_vertices.size() * sizeof(d7gfx::Mesh::Polygon::Vertex)));
        ...
        for (unsigned int i = 0; i < m_vertices.size(); i++) {
            ...
            m_vbo[i] = (d7gfx::Mesh::Polygon::Vertex) { { x, y, z }, { u, v }, { nx, ny, nz } };
        }
    }
} /* d7gfx */
